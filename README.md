...
The picker binds `/` to prompt for a case-insensitive message search, `enter` to use the selected session, `tab` to toggle between all folders and the folder of the scope, `shift-tab` and `ctrl-s` to make the folder of the selected session that scope, `ctrl-a` to walk the agent filter, `ctrl-r` to refresh the session list and preview, `esc` to clear an active search and to exit without one, and `ctrl-c` to exit.

The open picker keeps its list current by itself, shown as `↻ 5s` in the key list. Every 5 seconds it lists again from what it listed last: only the session files written since the previous look are read again - new sessions, sessions resumed just now and sessions that are working - and moved to where their time puts them, while every other row is taken over as it was and only gets its age and its live marker worked out anew, so a session that is merely open, finished or waiting at a prompt shows its current state. Which files were written is answered by a single `find -newer` against a marker file touched before each look, so nothing written while a list is put together is missed. Once a minute the list is made from scratch instead, for whatever the quick one leaves out: a file that is gone, a title a session got elsewhere. The cursor stays on its session when the rows move under it, as long as nothing is searched for - a query orders the rows by itself.
