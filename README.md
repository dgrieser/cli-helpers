...
**Usage:** `fzf-with-header [--command <command>] [--key-command <command>] [--key-descriptions <key>:<description>,...] [--reload-keys <key>,...] [--exec-reload-keys <key>,...] [--silent-keys <key>,...] [--exec-keys <key>,...] [--ask <key>:<question>] [--confirm <key>:<question>] [--ask-prefill <key>:<command>] [--footer-prefix <command>] [--watch <seconds>] [--watch-delay <seconds>] [--watch-while <command>] [--watch-interval-command <command>] [--columns <cols>] [--filter-columns <cols>] [--delimiter <delimiter>] [<query ...>] -- [fzf args]`
...
| `--ask-prefill <key>:<command>` | Command whose output the input line of an asking key starts with, so a value can be edited instead of typed again (requires `--ask` or `--confirm` for that key). It is run when the question opens and may use the placeholders of an `fzf` binding, e.g. `--ask-prefill 'ctrl-t:printf %s {1}'`. |
